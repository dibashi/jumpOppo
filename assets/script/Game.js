...
export default class Game extends cc.Component {
    ...
    _dirFirst = [1, 1, -1, 1, 1, -1, -1, 1, 1]; //引导阶梯

    onLoad() {
        toolsLog("--- Game onLoad  ---");
        ...
        //    toolsLog("--- 试验区 ---");
        //     let strTry1 = "123";
        //    toolsLog(JSON.parse(strTry1));
        //    toolsLog("-- type :" + typeof (strTry1) + " -- " + typeof (JSON.parse(strTry1)));
        //     let strTry2 = "{\"value\":123}";
        //    toolsLog(JSON.parse(strTry2));
        //    toolsLog("-- type :" + typeof (strTry2) + " -- " + typeof (JSON.parse(strTry2)));
        //    toolsLog("-- arr " + typeof (["aa"]));
    }
    ...
    start() {
        ...
        this.node.on(cc.Node.EventType.TOUCH_START, function (touch) {
            ...
            //在游戏中 且 不在加速状态 才能自由选择前进方向。
                ...
                //新手引导时屏蔽不正确的
                if (self.node_first.active) {
                    let isLeft = self.node_first.getChildByName("node_lead").x < 0;
                    let touchLeft = (touchPos.x < 360);
                    if (isLeft == touchLeft)
                        self.jumpRole(touchPos.x < 360);
                    else
                        return true;
        ...
        if (cc.dataMgr.userData.isRegame) {
            cc.dataMgr.userData.isReady = false;
            this.showGame();
        }
    ...
    initGame(isRelive) {
        ...
        if (!isRelive) {
            ...
            cc.dataMgr.userData.firstCountTime = 3;
            ...
            if (cc.dataMgr.userData.speedNum > 0) {
                if (cc.dataMgr.userData.isFirstIn == 0) //不是第一次才1.2 秒后自动跳
            ...
            if (cc.dataMgr.userData.isFirstIn > 0) {
                this.showLead();
                //cc.dataMgr.userData.isFirstIn = 0;
                // this.node_first.active = true;
                // this.node_first.getChildByName("node_first").active = true;
                // this.node_first.getChildByName("node_lead").active = false;
                // this.node_first.runAction(cc.repeat(cc.sequence(cc.delayTime(1), cc.callFunc(function () {
                //     --cc.dataMgr.userData.firstCountTime;
                //     if (cc.dataMgr.userData.firstCountTime == 0) {
                //         this.node_first.getChildByName("node_first").getChildByName("anniu_queding").active = true;
                //         this.node_first.getChildByName("node_first").getChildByName("lab_hintClose").getComponent(cc.Label).string = "任意点击关闭提示";
                //     }
                //     else {
                //         this.node_first.getChildByName("node_first").getChildByName("anniu_queding").active = false;
                //         this.node_first.getChildByName("node_first").getChildByName("lab_hintClose").getComponent(cc.Label).string = ("点击关闭:" + cc.dataMgr.userData.firstCountTime + "s");
                //     }
                // }, this)), 3));
            } else //音乐特殊处理
            ...
            //toolsLog("--- initGame relive ---" + aimY + " -- " + this.node_camera.y);
            //toolsLog(cc.dataMgr.userData);
            ...
            //toolsLog("-- 补 " + supr + " --R " + posBegin.y + " --B " + cc.dataMgr.userData.lastBoxY + " --C " + this.node_camera.y);
    ...
    jumpRole(isLeft) {
        ...
        toolsLog("aimPos-- " + cc.dataMgr.userData.aimRoleX + " -- " + cc.dataMgr.userData.aimRoleY);

        //判断角色复活后是否达到安全距离
        if (!cc.dataMgr.userData.isReliveDrop) {
            //toolsLog(this.node_camera.y + " -- " + cc.dataMgr.userData.aimRoleY);
    ...
    autoJump() {
        if (cc.dataMgr.userData.speedNum > 0) {
            ...
            //借助自动跳跃实现的 新手引导
            if (cc.dataMgr.userData.isFirstIn > 0 && cc.dataMgr.userData.isFirstIn < 8) {
                this.showLead();
            }
        }
        this.changeStreak();
    ...
    createBox(nodeN) {
        ...
        //前几次固定朝向
        //toolsLog("-- box -- " + cc.dataMgr.userData.isFirstIn + " -- " + cc.dataMgr.userData.countBox);
        if (cc.dataMgr.userData.isFirstIn > 0 && cc.dataMgr.userData.isFirstIn < 7 && cc.dataMgr.userData.countBox < 8) {
            isLeft = this._dirFirst[cc.dataMgr.userData.countBox] == 1 ? true : false;
            toolsLog("-- firstBox --" + isLeft);
        }
    ...
    getAimPos_o(aimPos) {
        ...
        for (let i = 0; i < this.root_box.children.length; ++i) {
            ...
            //目标砖块
            let dis = cc.v2(nodeN.x - aimPos.x, nodeN.y - aimPos.y).mag(); // nodeN.position.mag(aimPos);
            if (dis < 10) {
                //判断是否为障碍物
                let nodeNJs = nodeN.getComponent("NodeBox");
                if (nodeNJs) {
    ...
    useProp() {
        ...
        toolsLog("-- streakName --" + streakName);
        if (streakSf)
            this.node_streak.getComponent(cc.MotionStreak).texture = streakSf.getTexture();
        toolsLog("-- speedNum:" + cc.dataMgr.userData.speedNum + " -- " + cc.dataMgr.userData.cameraSpeedY);
    }
    ...
    //借助自动跳跃实现的 新手引导
    showLead() {
        toolsLog("-- showLoad --" + cc.dataMgr.userData.isFirstIn);
        if (cc.dataMgr.userData.isFirstIn > 0 && cc.dataMgr.userData.isFirstIn < 7) {
            this.node_first.active = true;
            this.node_game.getChildByName("node_hint").active = true;
            this.node_first.getChildByName("node_lead").active = true;
            this.node_first.getChildByName("node_first").active = false;
            ...
            this.node_first.getChildByName("node_lead").x = (isLeft ? -180 : 180);
            this.node_game.getChildByName("node_hint").getChildByName("node_left").active = (isLeft == true);
            this.node_game.getChildByName("node_hint").getChildByName("node_right").active = (isLeft == false);

            ++cc.dataMgr.userData.isFirstIn;
            cc.dataMgr.userData.speedNum = 1;
        } else {
            this.node_first.active = false;
            this.node_game.getChildByName("node_hint").active = false;
            cc.dataMgr.userData.isFirstIn = 0;
        }
        this._isInitGame = true;
        cc.dataMgr.userData.onGaming = false;
    ...
    lateUpdate(dt) {
        ...
        //     //toolsLog("-- countSecond -- " + this._countSecond + " -- " + this._countTime + " -- " + cc.dataMgr.userData.nextChangeTime);
        //     this.changeToNextBg();
        //     this._countSecond = 0;
        // }
    }
