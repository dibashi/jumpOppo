...
export default class PanelRandom extends cc.Component {
    ...
    _inRandom = false; //已经在 抽奖就不可以再抽了。

    onLoad() {
        //toolsLog("--- onLoad PanelRandom ---");
    ...
    initRand() {
        ...
        this._inRandom = false;
        toolsLog("--- initRand ---" + cc.dataMgr.isShowShare);
        ...
        if (cc.dataMgr.haveProp.freeTimes > 0 || cc.dataMgr.haveProp.rewardTimes > 0) {
            ...
            this.node_point.color = cc.Color.WHITE;
    ...
    //转盘结束奖励商品
    randEnd() {
        toolsLog("--- randEnd ---" + this._rewardName);
        //时间改变
        if (cc.dataMgr.haveProp.freeTimes > 0) {
    ...
    shareFriend() {
        ...
        //     });
        // } else {
        //     //toolsLog("-- Not is wechatGame Start --");
        //     cc.dataMgr.shareSuccess("startAd");
        // }
    }
}