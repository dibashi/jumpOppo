...
export default class NodeBox extends cc.Component {
    ...
    initBox(countBox, aimPos, isLeft, boxType) {
        //toolsLog("-- initBox:" + countBox + " -- " + aimPos.x + " -- " + aimPos.y);
        this._isAlive = true;
        this.node.stopAllActions();
        this.node.zIndex = 6000 - cc.dataMgr.userData.countBox;
        if (countBox > 5) {
    ...
    toDie() {
        if (this._isAlive) {
            ...
            let dis = cc.v2(cc.dataMgr.userData.aimRoleX - this.node.x, cc.dataMgr.userData.aimRoleY - this.node.y).mag();
            if (dis < 10) {
                //设置角色的死亡方式
